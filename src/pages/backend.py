...
class ciclos(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into ciclo values({0}, '{1}')".format(int(cicID),str(cicDesc)))
...
class cicloBycicID(Resource): # Deleta no BD de um usuário passado como parâmetro
...
class Grupos(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into grupo values({0}, {1},'{2}')".format(int(grpID),int(cicID),str(grpDesc)))
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        conn.execute("update grupo set cicID =%d " % int(cicID) +", grpDesc ='" + str(grpDesc) +
                    "'  where grpID =%d " % int(grpID))
...
class palavra_grupos(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into palavra_grupo values({0}, {1})".format(int(plvID),int(grpID)))

        query = conn.execute('select * from palavra_grupo order by plvID desc limit 1')
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        query = conn.execute("select * from palavra_grupo where plvID=%d " % int(plvID))
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)

class palavra_grupoByplvID(Resource): # Deleta no BD de um usuário passado como parâmetro
...
class Palavras(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into palavra values({0},'{1}','{2}','{3}',{4})".format(int(plvID),plvNome, plvclasse, plvSgn, int(dicID)))
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        conn.execute("update palavra set plvNome ='" + str(plvNome) +
                     "', plvclasse ='" + str(plvclasse) + "', plvSgn ='" + str(plvSgn) +"', dicID =" + int(dicID) + "  where plvID =%d " % int(plvID))

        query = conn.execute("select * from palavra where plvID=%d " % int(plvID))
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)

class palavraByplvID(Resource): # Deleta no BD de um usuário passado como parâmetro
    def delete(self, plvID):
        conn = db_connect.connect()
...
class pergunta_grupo(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into pergunta_grupo values({0}, {1})".format(int(pgtID),int(grpID)))

        query = conn.execute('select * from pergunta_grupo order by pgtID desc limit 1')
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        query = conn.execute("select * from pergunta_grupo where pgtID=%d " % int(pgtID))
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)

class pergunta_grupoBypgtID(Resource): # Deleta no BD de um usuário passado como parâmetro
...
class Perguntas(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into pergunta values( {0},'{1}','{2}','{3}','{4}','{5}','{6}','{7}')".format(pgtID,pgtDes, pgtRes,pgtAltFls1,pgtAltFls2,pgtAltFls3,pgtTipo, pgtDif))
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        conn.execute("update pergunta set pgtDes ='" + str(pgtDes) +
                     "', pgtRes ='" + str(pgtRes) +"', pgtAltFls1 ='" + str(pgtAltFls1) +"', pgtAltFls2 ='" + str(pgtAltFls2) +"', pgtAltFls3 ='" + str(pgtAltFls3) +"', pgtTipo ='" + str(pgtTipo) +"', pgtDif ='" + str(pgtDif) + "'  where pgtID =%d " % int(pgtID))

        query = conn.execute("select * from pergunta where pgtID=%d " % int(pgtID))
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)

class perguntaBypgtID(Resource): # Deleta no BD de um usuário passado como parâmetro
    def delete(self, pgtID):
        conn = db_connect.connect()
...
class usuario_ciclos(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into usuario_ciclo values({0},{1},'{2}')".format(userID,int(cicID), situacao))

        query = conn.execute('select * from usuario_ciclo order by userID desc limit 1')
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        conn.execute("update usuario_ciclo set cicID =%d" % int(cicID) +
                     ", situacao ='" + str(situacao) +"'  where userID = %d " % int(userID))

        query = conn.execute("select * from usuario_ciclo where userID= %d " % int(userID))
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)

class usuario_cicloByuserID(Resource): # Deleta no BD de um usuário passado como parâmetro
...
class Usuarios(Resource):
    ...
    def post(self): # Inclui no BD um usuário passado como parâmetro
        ...
        conn.execute(
            "insert into usuario values({0},'{1}','{2}','{3}')".format(userID,email,nome, senha))
    ...
    def put(self): # Update*(atualizar) no BD de um usuário passado como parâmetro
        ...
        conn.execute("update usuario set nome ='" + str(nome) +
                     "', senha ='" + str(senha) + "', email ='" + str(email) +"' where userID = %d " % int(userID))

        query = conn.execute("select * from usuario where userID= %d " % int(userID))
        result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
        return jsonify(result)
...
class usuarioByuserID(Resource): # Deleta no BD de um usuário passado como parâmetro
    def delete(self, userID):
        conn = db_connect.connect()
